"""Backtest Expert scoring (ported verbatim from ibd — broker-agnostic).
...
def score_from_trades(df: pd.DataFrame, symbol: str, strategy: str = "options",
                      since=None) -> BacktestScore:
    """Derive a BacktestScore from a normalized trade DataFrame for one symbol."""
    ...
    empty = BacktestScore(symbol=symbol, strategy=strategy, total_trades=0, win_rate=0.0,
                          profit_factor=0.0, max_drawdown_pct=100.0, years_tested=0.0,
                          windowed=_windowed).compute()
    ...
    return BacktestScore(symbol=symbol, strategy=strategy, total_trades=n, win_rate=win_rate,
                         profit_factor=round(pf, 3), max_drawdown_pct=round(dd_pct, 1),
                         years_tested=round(years, 1), windowed=_windowed).compute()
