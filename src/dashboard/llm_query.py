...
- Live open IBKR orders: orders already submitted to IBKR and pending execution \
(symbol, secType, right, strike, expiry, action, qty, remaining, limit price, status). \
Status "ValidationError" with errorCode 399 means the order is accepted and queued — \
it will be placed at the exchange when the market opens; this is NOT a failure.
- Suggested Orders from the Orders tab: the user's pending order queue generated by derive.py. \
These are ready-to-submit orders, NOT vague recommendations. \
Cover = sell covered calls on assigned stock; \
Sow = sell naked puts/calls to open new positions; \
Reap = buy-to-close profitable open options; \
Protect = buy puts for downside hedging. \
Each order row includes symbol, right (C/P), strike, expiry, dte, qty (contracts), \
undPrice, xPrice (expected execution price). xPrice × qty × 100 = dollar value per row. \
CRITICAL — when answering "do I have a covered call / sow / reap order for symbol X?": \
check BOTH the Live open IBKR orders section AND the matching Suggested Orders section \
(Cover for CCs, Sow for puts/calls). A symbol present in either source means an order \
exists for it. Do NOT say "no order exists" if you only checked one source. \
