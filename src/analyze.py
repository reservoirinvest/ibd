...
df = pd.concat([df_pf.assign(source="pf"), df_openords.assign(source="oo")], ignore_index=True)

df = df.assign(dte=df.expiry.apply(lambda x: get_dte(x) if pd.notna(x) and x else np.nan))
...
df = (
    pd.concat([df, df_unds.assign(source="und")], ignore_index=True)
...
cover_projection = (
    (df_reward.dte.mean() / 7) * abs(df_reward.premium.sum()) if not df_reward.empty else 0
)
...
if not df_risk.empty:
    risk_msg.append(
        f"Risk from {df_pf[df_pf.state == 'protecting'].symbol.nunique()} protected stocks valued at ${stocks_val:,.0f} is ${df_risk.unprot_val.sum():,.0f} for {df_risk.dte.mean():.1f} days."
    )
    risk_msg.append(f"Risk premium paid: ${df_risk.cost.sum():,.0f}")

unprotected_stocks = df[
    (df.source == "und") & (df.state.isin(["unprotected", "exposed"]))
].symbol.unique()
...
podf_mkt = (
    df_pf[df_pf.symbol.isin(podf.symbol.unique()) & (df_pf.secType == "STK")].mktVal.sum()
    if not podf.empty
    else 0
)
...
if unprotected_stocks.size > 0:
    stocks_str = "\n\t".join(
        [", ".join(unprotected_stocks[i : i + 5]) for i in range(0, len(unprotected_stocks), 5)]
    )
    ...
    if df_protect is not None:
        ...
        protection_price = (
            (dprot.xPrice * dprot.qty * 100).sum() if "xPrice" in dprot.columns else 0
        )
...
print("=====================")
state_counts = (
    ", ".join(f"{state}: {len(df)}" for state, df in df_unds.groupby("state"))
    if not df_unds.empty
    else "No states available"
)
print(state_counts)
...
if df_unds is not None and not df_unds.empty:
    ...
    de_orphed_symbols = (
        set(df_deorph["symbol"]) if df_deorph is not None and not df_deorph.empty else set()
    )
...
unique_oo_symbols = (
    df_openords["symbol"].nunique()
    if not df_openords.empty and "symbol" in df_openords.columns
    else 0
)
unique_pf_symbols = (
    df_pf["symbol"].nunique() if not df_pf.empty and "symbol" in df_pf.columns else 0
)
...
if not chains.empty and not df_unds.empty:
    ...
    missing_in_chains_from_pf = (
        df_pf[~df_pf["symbol"].isin(chains["symbol"])]["symbol"].unique() if not df_pf.empty else []
    )
